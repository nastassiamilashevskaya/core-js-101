...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (isStartIncluded && isEndIncluded) return `[${result}]`;
  if (!isStartIncluded && isEndIncluded) return `(${result}]`;
  if (isStartIncluded && !isEndIncluded) return `[${result})`;
  if (!isStartIncluded && !isEndIncluded) return `(${result})`;
  return result;
...
function getCommonDirectoryPath(pathes) {
  ...
  const first = splitPathes[0].map((e, item) => splitPathes.map((i) => i[item]));
  const equalElements = first.filter((array) => array.every((e) => e === array[0]));
  return [...equalElements, ['']].map((item) => item[0]).join('/');
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    if (position[0][i] === position[1][i] && position[0][i] === position[2][i]) {
      if (position[0][i] !== x) return position[0][i];
    }
    if (position[i][0] === position[i][1] && position[i][0] === position[i][2]) {
      if (position[i][0] !== x) return position[i][0];
    }
  }

  if (position[0][0] === position[1][1] && position[0][0] === position[2][2]) {
    if (position[1][1] !== x) return position[1][1];
  }
