...
function getPolynom(...args) {
  const polynomLength = args.length;
  return (x) => {
    if (polynomLength === 3) return (args[0] * (x ** 2) + args[1] * x + args[2]);
    if (polynomLength === 2) return (args[0] * x + args[1]);
    if (polynomLength === 1) return (args[0]);
    return x;
  };
}
